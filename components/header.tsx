...
import GroupMemberRequest from '@/app/notification/components/groupMemberRequest';
import { MembershipRequestStatus } from '@prisma/client';
import GroupMemberApprovals from '@/app/notification/components/groupMemberApprovals';
...
const Header = () => {
  ...
  const [overlayNotificationTarget, setOverlayNotificationTarget] = useState(null);
  ...
  const handleNotificationIconClick = (event: any) => {
    setShowNotificationOverlay(!showNotificationOverlay);
    setOverlayNotificationTarget(event.target);
  }
      ...
      <Container>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          ...
          <Nav className="ms-2 me-2">
            ...
            <Nav.Link ref={notificationRef} onClick={handleNotificationIconClick}>
              {session?.user.notificationCount > 0 ? (
              ...
              <Overlay show={showNotificationOverlay} target={notificationRef} placement="bottom" container={notificationRef} containerPadding={20}>
                <Popover id="popover-contained" className={styles.notificationPopover}>
                  {session?.user.membershipProcessedBys?.filter((s: any) => s.membershipRequestStatus === MembershipRequestStatus.REQUESTED)?.length > 0 && (<> <Popover.Header className='bg-dark text-center' as="h3">그룹 가입 요청</Popover.Header>
                    <Popover.Body>
                      <GroupMemberRequest requests={session?.user.membershipProcessedBys?.filter((s: any) => s.membershipRequestStatus === MembershipRequestStatus.REQUESTED)} />
                    </Popover.Body></>)}
                  {session?.user.membershipRequestedBys.filter((s: any) => s.membershipRequestStatus === MembershipRequestStatus.APPROVED)?.length > 0 && (<> <Popover.Header className='bg-dark text-center' as="h3">그룹 승인</Popover.Header>
                    <Popover.Body>
                      <div>
                        <GroupMemberApprovals approvals={session?.user.membershipRequestedBys.filter((s: any) => s.membershipRequestStatus === MembershipRequestStatus.APPROVED)} />
                      </div>
                    </Popover.Body></>)}
                </Popover>
              </Overlay>
